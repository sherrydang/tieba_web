...
public final class BigDecimalUtil {
	...
	/**
	 * Convenience method to create a BigDecimal with a String, can be
	 * statically imported.
	 * 
	 * @param val
	 *            a string representing the BigDecimal
	 * @return
	 */
	...
	/**
	 * @return numerator / denominator if they are not null and the denominator
	 *         is not zero, it returns null otherwise.
	 */
	...
	/**
	 * returns a new BigDecimal with correct scale after being round to n dec
	 * places.
	 * 
	 * @param bd
	 *            value
	 * @param numberOfDecPlaces
	 *            number of dec place to round to
	 * @param finalScale
	 *            final scale of result (typically numberOfDecPlaces &lt;
	 *            finalScale);
	...
	/**
	 * Calculate the weight of the constituent and add it to the running
	 * weighted value. runningWeightedVal + valueToAdd * weightForValueToAdd /
	 * totalWeight
	 * 
	...
	public static BigDecimal addWeightedConstituent(final BigDecimal runningWeightedVal, final BigDecimal valueToAdd, final BigDecimal weightForValueToAdd, final BigDecimal totalWeight) {
		return BigDecimalUtil.doAdd(runningWeightedVal,
				BigDecimalUtil.divide(BigDecimalUtil.multiply(valueToAdd, BigDecimalUtil.abs(weightForValueToAdd)), BigDecimalUtil.abs(totalWeight), BigDecimal.ROUND_HALF_UP));
	}
	...
	/**
	 * Returns a value rounded to p digits after decimal. If p is negative, then
	 * the number is rounded to places to the left of the decimal point. eg.
	 * 10.23 rounded to -1 will give: 10. If p is zero, the returned value is
	 * rounded to the nearest integral value.
	 * <p>
	 * If n is negative, the resulting value is obtained as the round-up value
	 * of absolute value of n multiplied by the sign value of n (@see
	...
	/**
	 * Returns a value rounded-up to p digits after decimal. If p is negative,
	 * then the number is rounded to places to the left of the decimal point.
	 * eg. 10.23 rounded to -1 will give: 20. If p is zero, the returned value
	...
	/**
	 * Return a negative amount based on amount if true, otherwise return the
	 * ABS.
	 */
	...
	/**
	 * Move by 2 DEC place to the left and take the long value, this takes care
	 * of values like 0.18 in fractions.
	 */
