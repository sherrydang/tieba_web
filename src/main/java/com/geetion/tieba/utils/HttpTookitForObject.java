...
public class HttpTookitForObject {
	/**
	 * 从URL中下载文件
	 * 
	 * @param url
    ...
    public static String urlConnectionPostObjects(String tourl, Map<String, Object> paramSet) {
        BufferedReader in = null;
        ...
        try {
            ...
            // 当存在post的值时，才打开OutputStreamWriter
            if (paramSet != null) {
                for (Entry<String, Object> set : paramSet.entrySet()) {
                    data.add(set.getValue());
                }
            }
            if (data != null && data.size() > 0) {
                wr = new ObjectOutputStream(conn.getOutputStream());
                wr.writeObject(data);
                wr.flush();
            }
            // Get the response
